...
#if defined(XRUI_DLL)
#define XRUI_EXTERN __declspec(dllexport)
#else
#define XRUI_EXTERN
#endif
#else
...
#ifdef WIN32
        XRUI_EXTERN DWORD ConvertToNative(KeyCodes key);
        XRUI_EXTERN KeyCodes ConvertToKeyCode(DWORD key);
#elif __APPLE__
        XRUI_EXTERN CGKeyCode ConvertToNative(KeyCodes key);
        XRUI_EXTERN KeyCodes ConvertToKeyCode(CGKeyCode key);
#elif __linux__
        XRUI_EXTERN KeyCode ConvertToNative(Input_Lite::KeyCodes key);
        XRUI_EXTERN Input_Lite::KeyCodes ConvertToKeyCode(KeyCode key);
#else
#error "Unknown Operating System!"
#endif
        ...
        struct input : INPUT {
            ...
            input(bool p, KeyCodes k);
            input(int x, int y, bool abs);
            input(bool p, MouseButtons k);
        };
        ...
        template <class T>
        XRUI_EXTERN void SendInput(const T& e);

        XRUI_EXTERN void SendInput(const KeyEvent& e);
        XRUI_EXTERN void SendInput(const MouseButtonEvent& e);
        XRUI_EXTERN void SendInput(const MouseScrollEvent& e);
        XRUI_EXTERN void SendInput(const MousePositionOffsetEvent& e);
        XRUI_EXTERN void SendInput(const MousePositionAbsoluteEvent& e);

        class XRUI_EXTERN IInputManager {
        public:
        ...
        class XRUI_EXTERN IInputConfiguration {
        public:
        ...
        XRUI_EXTERN std::shared_ptr<IInputConfiguration> CreateInputConfiguration();
        */
    } // namespace xrui
} // namespace bonus
