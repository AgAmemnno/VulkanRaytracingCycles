...
macro(compile_flag_RTC)
add_compile_options(
  "$<$<CONFIG:DEBUG>:-${MULTIMODE};>"
)
...
macro(add_cc_flags_custom_test
 ...
 # message(STATUS  "CMAKE FLAGS[${name}]   c++  ${CMAKE_CXX_FLAGS}     c ${CMAKE_C_FLAGS}   ")
...
function(blender_add_lib__impl
 ...
 if(  ( ${name} MATCHES "extern_(.*)" )   AND  ( NOT BL_PREBUILD STREQUAL "" ) )

  set(${name}  ${name}.lib  CACHE STRING "external_lib")
  message(STATUS   "BL_EXTERN_PREBUILD ${name}")
...
message(STATUS   ">>>>>>>>>>>>>>>>>>>>>>>>   Add Library  ${name}")
  ...
  target_global_lib(${name} "")



  #set_target_properties(${name}
    # PROPERTIES
      #    LIBRARY_OUTPUT_DIRECTORY_${OUTPUTCONFIG}    ${BINARY_DIR_P}/Lib
       #   RUNTIME_OUTPUT_DIRECTORY_${OUTPUTCONFIG}  ${BINARY_DIR_P}/Lib
   #)
...
# only MSVC uses SOURCE_GROUP
function(blender_add_exe__impl
  name
  sources
  includes
  includes_sys
 ...
 if(  ( ${name} MATCHES "extern_(.*)" )   AND  ( NOT BL_PREBUILD STREQUAL "" ) )
  ...
  set(${name}  ${name}.exe  CACHE STRING "external_exe")
  message(STATUS   "BL_EXTERN_PREBUILD ${name}")

else()

  add_executable(${name} ${sources})
  target_global_exe(${name})

  if(NOT "${library_deps}" STREQUAL "")
    set(next_library_mode "")
    target_link_directories(${name}  PUBLIC ${CMAKE_BINARY_DIR}/lib/${CMAKE_CONFIGURATION_TYPES})
    message("exex link   ${library_deps}")
    foreach(library ${library_deps})
      string(TOLOWER "${library}" library_lower)
      ...
      if(("${library_lower}" STREQUAL "optimized") OR
        ...
        elseif("${next_library_mode}" STREQUAL "debug")
          ...
          #target_link_libraries(${name} INTERFACE ${library}.lib)
          target_link_libraries(${name} PUBLIC ${library}.lib)
...
function(blender_add_exe
  ...
  blender_add_exe__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")
...
function(blender_add_test_lib
  ...
  message(STATUS "Test libarary  "  ${includes}  "     "     ${includes_sys})
  blender_add_lib__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")
...
macro(setup_platform_linker_flags)
  ...
  message(STATUS   "LINKER flas   ${CMAKE_EXE_LINKER_FLAGS_DEBUG}    ")
...
function(msgfmt_simple
  ...
  get_filename_component(_file_to_path ${_file_to} PATH)
  #set(outBinDir "${CMAKE_BINARY_DIR}/bin/${CMAKE_CONFIGURATION_TYPES}")
  set(outBinDir "${BINARY_DIR}")
  set(cmd ${outBinDir}/msgfmt.exe)
    if(NOT EXISTS  ${cmd})
       message(FATAL_ERROR  " msgfmt not exist     ${outBinDir}/msgfmt.exe  " )
    endif()
  ...
  add_custom_command(
    OUTPUT  ${_file_to}
    COMMAND ${CMAKE_COMMAND} -E make_directory ${_file_to_path}
    COMMAND   ${cmd} ${_file_from} ${_file_to}
