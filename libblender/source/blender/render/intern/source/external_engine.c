...
static void render_result_to_bake(RenderEngine *engine, RenderResult *rr)
{
  ...
  /*   Cop1y from tile render result to full image bake result. */
  ...
  /*
  float* tpixel = MEM_mallocN(h * w * engine->bake.depth * sizeof(float), __func__);
  int tidx =  1024 / w * 1024 / h;
  static int  call  = 0;
  */
  ...
  for (int ty = 0; ty < h; ty++) {
    ...
    /*
    float* ptr = tpixel + offset;
    for (int i = 0; i < w * engine->bake.depth; i+=4) {
        ptr[i]      =  (float)(call)/(float)tidx;
        ptr[i+1]   = (float)(call) / (float)tidx;
        ptr[i+2]  = (float)(call) / (float)tidx;
        ptr[i+3]  = 1.0f;
    }
    memcpy(engine->bake.result + bake_offset, ptr, size);
    */
  }
