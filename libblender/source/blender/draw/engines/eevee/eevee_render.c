...
static redisContext* rd_ctx = NULL;

void  RedisCli_SelectDB(int n) {
        static int  currenyDB = 0;
        if (n != currenyDB) {

            redisReply* reply;
            reply = (redisReply*)redisCommand(rd_ctx, "SELECT %d", n);
            RD_CHECK(reply);
            freeReplyObject(reply);
            currenyDB = n;
        };
};
void RedisCli_FlushDB(int n) {
    ...
    reply = (redisReply*)redisCommand(rd_ctx, "FLUSHDB");
    RD_CHECK(reply);
    freeReplyObject(reply);

};
void RedisCli_connect() {

    if (rd_ctx != NULL)return;
    ...
    if (isunix) {
        rd_ctx = redisConnectUnixWithTimeout(hostname, timeout);
    }
    else {
        rd_ctx = redisConnectWithTimeout(hostname, port, timeout);
    }
    if (rd_ctx == NULL || rd_ctx->err) {
        if (rd_ctx) {
...
void RedisCli_disconnect() {
    if (rd_ctx != NULL)redisFree(rd_ctx);
    rd_ctx = NULL;
}
void RedisCli_floatarray(float*  data,size_t size,const char* name) {

    if (!data) return;
   


    printf(" dump float array [%s]  ===============>>>    \n ",name);
    for (int i = 0; i < size; i++) {
        printf("  %.4f  ", data[i]);
    }
    printf("\n");

    const char* KEY = "EV::render";
    char* ptr = (char*)data;

#define struct_enc(ptr, str,_name)\
    {\
    size_t out_size = 0;\
    char* _name = Base64_Encode((char*)ptr, sizeof(float) * size, &out_size);\
   RD_CHECK(redisCommand(rd_ctx, "HMSET %s %s %b ", KEY, name, _name,out_size));\
  free(_name);\
}
  
    struct_enc(data, size, __floatarray);
...
void EEVEE_render_view_sync(EEVEE_Data *vedata, RenderEngine *engine, struct Depsgraph *depsgraph)
{
  ...
  RedisCli_connect();
  RedisCli_FlushDB(6);
  RedisCli_floatarray((float*)viewmat ,16,"viewmat");
  RedisCli_floatarray((float*)winmat, 16, "winmat");
  RedisCli_disconnect();
#endif
