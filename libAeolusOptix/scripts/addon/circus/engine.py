...
def _workaround_buggy_drivers():
    if _is_using_buggy_driver():
        import _circus
        if hasattr(_circus, "opencl_disable"):
            print("Circus: OpenGL driver known to be buggy, disabling OpenCL platform.")
            _circus.opencl_disable()


def _configure_argument_parser():
    import argparse
    # No help because it conflicts with general Python scripts argument parsing
    parser = argparse.ArgumentParser(description="Circus Addon argument parser",
                                     add_help=False)
    parser.add_argument("--circus-resumable-num-chunks",
                        help="Number of chunks to split sample range into",
                        default=None)
    parser.add_argument("--circus-resumable-current-chunk",
                        help="Current chunk of samples range to render",
                        default=None)
    parser.add_argument("--circus-resumable-start-chunk",
                        help="Start chunk to render",
                        default=None)
    parser.add_argument("--circus-resumable-end-chunk",
                        help="End chunk to render",
                        default=None)
    parser.add_argument("--circus-print-stats",
...
def _parse_command_line():
    ...
    if args.circus_resumable_num_chunks is not None:
        if args.circus_resumable_current_chunk is not None:
            import _circus
            _circus.set_resumable_chunk(
                int(args.circus_resumable_num_chunks),
                int(args.circus_resumable_current_chunk),
            )
        elif args.circus_resumable_start_chunk is not None and \
                args.circus_resumable_end_chunk:
            import _circus
            _circus.set_resumable_chunk_range(
                int(args.circus_resumable_num_chunks),
                int(args.circus_resumable_start_chunk),
                int(args.circus_resumable_end_chunk),
            )
    if args.circus_print_stats:
        import _circus
        _circus.enable_print_stats()
...
def init():
    ...
    _circus.init(path, user_path, bpy.app.background)
    _parse_command_line()
...
def render(engine, depsgraph):
    import _circus
    if hasattr(engine, "session"):
        _circus.render(engine.session, depsgraph.as_pointer())
...
def bake(engine, depsgraph, obj, pass_type, pass_filter, width, height):
    ...
    if session is not None:
        _circus.bake(engine.session, depsgraph.as_pointer(), obj.as_pointer(), pass_type, pass_filter, width, height)
...
def reset(engine, data, depsgraph):
    ...
    if prefs.experimental.use_circus_debug and prefs.view.show_developer_ui:
        _circus.debug_flags_update(depsgraph.scene.as_pointer())
    else:
        _circus.debug_flags_reset()

    data = data.as_pointer()
    depsgraph = depsgraph.as_pointer()
    _circus.reset(engine.session, data, depsgraph)


def sync(engine, depsgraph, data):
    import _circus
    _circus.sync(engine.session, depsgraph.as_pointer())
...
def draw(engine, depsgraph, region, v3d, rv3d):
    ...
    # draw render image
    _circus.draw(engine.session, depsgraph, v3d, rv3d)


def available_devices():
    import _circus
    return _circus.available_devices()


def with_osl():
    import _circus
    return _circus.with_osl


def with_network():
    import _circus
    return _circus.with_network


def system_info():
    import _circus
    return _circus.system_info()
...
def list_render_passes(scene, srl):
    ...
    if srl.circus.use_pass_crypto_asset:
        for i in range(0, crypto_depth):
...
def detect_conflicting_passes(scene, view_layer):
    ...
    for aov in view_layer.circus.aovs:
