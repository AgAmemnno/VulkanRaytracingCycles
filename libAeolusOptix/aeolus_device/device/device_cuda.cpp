...
/*
bool device_cuda_vulkan(int  devId) {
    ...
    for (int i = 0; i < deviceCount; ++i) {
        CUdevprop devProp = { };
        const CUresult result = cuDeviceGetProperties(&devProp, i);
        if ((result == CUDA_SUCCESS) & (devId == i)) {
            cudaDevice = i;
            break;
        }
    }
    ...
    result = cuDeviceGetAttribute(&deviceComputeMode, CU_DEVICE_ATTRIBUTE_COMPUTE_MODE, cudaDevice);
    if (result != CUDA_SUCCESS) {
        fprintf(stderr, "Error getting devices: %s ", cuewErrorString(result));
    }
    result = cuDeviceGetAttribute(&attributeVal, CU_DEVICE_ATTRIBUTE_VIRTUAL_ADDRESS_MANAGEMENT_SUPPORTED, cudaDevice);
    if (result != CUDA_SUCCESS) {
        fprintf(stderr, "Error getting devices: %s", cuewErrorString(result));
    }
...
#else
    result = cuDeviceGetAttribute(&deviceSupportsHandle, CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_WIN32_HANDLE_SUPPORTED, cudaDevice);
    if (result != CUDA_SUCCESS) {
        fprintf(stderr, "Error getting devices: %s", cuewErrorString(result));
    }
#endif
