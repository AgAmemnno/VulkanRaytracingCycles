...
#define CYCLES_CUDA_NVCC_EXECUTABLE "C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v10.1\\bin\\nvcc.exe"
...
#if CUDA_VER == 10
  // TODO 11.1   P2P   Ensure array access over the link is possible as well (for 3D textures)
...
void CUDADevice::tex_alloc(device_texture &mem)
{
  ...
  if (slot >= texture_info.size()) {
    ...
    for (int i = slot; i < 128; i++) texture_info[i].width = (uint)-1;
  }
...
#ifdef DUMP_CUDA
  texture_info[slot].cl_buffer = redis->textureStore(mem, texDesc, resDesc);
#endif
...
void CUDADevice::render(DeviceTask &task, RenderTile &rtile, device_vector<WorkTile> &work_tiles)
{
  ...
  uint step_samples = (uint)divide_up(min_blocks * num_threads_per_block, wtile->w * wtile->h);
  ...
  for (int sample = start_sample; sample < end_sample; sample += step_samples) {
    ...
    uint num_blocks = (uint)divide_up(total_work_size, num_threads_per_block);
...
class CUDASplitKernelFunction : public SplitKernelFunction {
 ...
 public:
  ...
  bool enqueue(const KernelDimensions &dim, void *args[])
  {
    ...
    int xblocks = ( (int)(dim.global_size[0] * dim.global_size[1]) + threads_per_block - 1) /
                  threads_per_block;
                                                     ...
                                                     device_memory &work_pool_wgs)
{
  ...
  int end_sample = rtile.start_sample + rtile.num_samples;
  int queue_size = (int)(dim.global_size[0] * dim.global_size[1]);
                                               ...
                                               DeviceTask & /*task*/)
{
  ...
  int2 global_size = make_int2( (int)side, (int)round_down(num_elements / side, 16));
