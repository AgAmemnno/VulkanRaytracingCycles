      ...
      device(device)
{
  ...
  tile_info->num_frames = min((int)task.denoising_frames.size() + 1, DENOISE_MAX_FRAMES);
...
void DenoisingTask::setup_denoising_buffer()
{
  ...
  buffer.stride = (int)(align_up((size_t)buffer.width, 4));
  buffer.h = rect.w - rect.y;
  int alignment_floats = (int)divide_up(device->mem_sub_ptr_alignment(), sizeof(float));
  buffer.pass_stride = (int)align_up(buffer.stride * buffer.h, alignment_floats);
  ...
  int mem_size = (int)align_up(tile_info->num_frames * buffer.frame_stride + 4, alignment_floats);
