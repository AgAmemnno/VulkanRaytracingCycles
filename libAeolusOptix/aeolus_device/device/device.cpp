                         ...
                         const DeviceDrawParams &draw_params)
{
  ...
  mem_copy_from(rgba, y, w, h, (int)rgba.memory_elements_size(1));
  ...
  if (use_fallback_shader) {
    if (!bind_fallback_display_space_shader( (float)dw, (float)dh)) {
      return;
    }
  ...
  foreach (const DeviceInfo &device, subdevices) {
    ...
    if (device.type == DEVICE_CPU && subdevices.size() > 1) {
      if (background) {
        ...
        int cpu_threads = max(orig_cpu_threads - ((int)subdevices.size() - 1), 0);
