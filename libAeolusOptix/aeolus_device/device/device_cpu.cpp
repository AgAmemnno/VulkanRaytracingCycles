...
#undef KERNEL_FUNCTIONS
...
#ifdef DUMP_SD_BUFFERS
    printf("Debug redis dump device_cpu activated. \n");
#endif
  ...
  void mem_alloc(device_memory &mem)
  {
    if (mem.type == MEM_TEXTURE) {
      ...
      if (mem.type == MEM_DEVICE_ONLY) {
        ...
        void *data = util_aligned_malloc(mem.memory_size(), (int)alignment);
  ...
  {
    //VLOG(1)
    std::cout   << "Global memory allocate: " << mem.name << ", "
            << string_human_readable_number(mem.memory_size()) << " bytes. ("
            << string_human_readable_size(mem.memory_size()) << ") \n";
                                 ...
                                 DenoisingTask *task)
  {
    ...
    int w = (int)align_up(rect.z - rect.x, 4);
    int h = rect.w - rect.y;
  ...
  bool adaptive_sampling_filter(KernelGlobals *kg, RenderTile &tile, int sample)
  {
    ...
    if (!kernel_data.integrator.adaptive_stop_per_sample) {
      for (int y = (int)wtile.y; y < (int)(wtile.y + wtile.h); ++y) {
        for (int x =(int)wtile.x; x <(int) (wtile.x + wtile.w ); ++x) {
  ...
  void adaptive_sampling_post(const RenderTile &tile, KernelGlobals *kg)
  {
    ...
    for (int y = (int)tile.y; y <(int)( tile.y + tile.h ); y++) {
      for (int x = (int)tile.x; x < (int) (tile.x + tile.w ); x++) {
  ...
  void denoise_openimagedenoise(DeviceTask &task, RenderTile &rtile)
  {
    if (task.type == DeviceTask::DENOISE_BUFFER) {
      ...
      const float invscale =(float) rtile.sample;
      ...
      if (ntile.buffer) {
        ...
        for (int y = ymin; y < ymax; y++) {
          for (int x = 0; x <  (int)(pass_stride) * (xmax - xmin); x += (int)pass_stride) {
...
class CPUSplitKernelFunction : public SplitKernelFunction {
 public:
                       ...
                       device_memory &data)
  {
    ...
    kg->global_size = make_int2((int)dim.global_size[0], (int)dim.global_size[1]);
  ...
  for (int y = 0; y < dim.global_size[1]; y++) {
    for (int x = 0; x < dim.global_size[0]; x++) {
      ...
      device->data_init_kernel()((KernelGlobals *)kernel_globals.device_pointer,
                                 ...
                                 (int)(dim.global_size[0] * dim.global_size[1]),
