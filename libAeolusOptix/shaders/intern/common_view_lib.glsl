...
///MODIFY layout(std140) uniform viewBlock
layout(set = 0, binding = 0)uniform co_vi_viewBlock
{
...
#    define instanceId gl_InstanceIndex
    ...
    layout(set = 0, binding = 1)uniform co_vi_optionalBlock
    {
        int resourceChunk;
      ...
      * the fragment shader uses the resource_id. */
      ...
      #    define RESOURCE_ID_VARYING(id) layout (location = id) out flat int resourceIDGeom;
      #    define PASS_RESOURCE_ID resourceIDGeom = resource_id;
      #else
      #    define RESOURCE_ID_VARYING(id) layout (location = id) out flat int resourceIDFrag;
...
#  define RESOURCE_ID_VARYING(id) \
   layout (location = id) flat out int resourceIDFrag; \
   layout (location = id) flat in int resourceIDGeom[];
...
layout(set = 1, binding = 0)uniform co_vi_modelBlock
{
