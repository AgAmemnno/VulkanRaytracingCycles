...
#  elif defined(__OBJECT_MOTION__)
#    define BVH_FUNCTION_NAME bvh_intersect_motion
#    define BVH_FUNCTION_FEATURES BVH_MOTION
#    include "kernel/bvh/bvh_traversal.h.glsl"
#  elif defined(__HAIR__)
...
ccl_device_inline float3 ray_offset(float3 P, float3 Ng)
{
  ...
  if (fabsf(P.x) < epsilon_test) {
    ...
    ix += ( bool((ix ^ __float_as_uint(Ng.x)) >> 31) )? -epsilon_i : epsilon_i;
  ...
  if (fabsf(P.y) < epsilon_test) {
    ...
    iy += (bool((iy ^ __float_as_uint(Ng.y)) >> 31)) ? -epsilon_i : epsilon_i;
  ...
  if (fabsf(P.z) < epsilon_test) {
    ...
    iz += (bool((iz ^ __float_as_uint(Ng.z)) >> 31)) ? -epsilon_i : epsilon_i;
