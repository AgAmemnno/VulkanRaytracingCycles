...
ccl_device void svm_node_vector_rotate(uint input_stack_offsets,
                                       ...
                                       uint result_stack_offset)
{
  ...
  svm_unpack_node_uchar4(input_stack_offsets, (type), (vector_stack_offset), (rotation_stack_offset), (invert));

  svm_unpack_node_uchar3(axis_stack_offsets, (center_stack_offset), (axis_stack_offset), (angle_stack_offset));

  if (stack_valid(result_stack_offset)) {

    float3 vector = stack_load_float3(vector_stack_offset);
    float3 center = stack_load_float3(center_stack_offset);
    float3 result = make_float3(0.0f, 0.0f, 0.0f);

    if (type == NODE_VECTOR_ROTATE_TYPE_EULER_XYZ) {
      float3 rotation = stack_load_float3(rotation_stack_offset);  // Default XYZ.
      ...
      switch (type) {
        ...
        case NODE_VECTOR_ROTATE_TYPE_AXIS_Z:
          ...
          break;
        default:
          axis = normalize(stack_load_float3(axis_stack_offset));
          break;
      }
      float angle = stack_load_float(angle_stack_offset);
      angle = bool(invert) ? -angle : angle;
      result = (len_squared(axis) != 0.0f) ?rotate_around_axis(vector - center, axis, angle) + center: vector;
    }
