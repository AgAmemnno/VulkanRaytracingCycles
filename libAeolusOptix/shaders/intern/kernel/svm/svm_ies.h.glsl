...
ccl_device_inline float interpolate_ies_vertical(
    inout KernelGlobals kg, int ofs, int v, int v_num, float v_frac, int h)
{
  /* Since lookups are performed in_rsv spherical coordinates, clamping the coordinates at the low end
   * of v (corresponding to the north pole) would result in_rsv artifacts. The proper way of dealing
...
#define IES_LOOKUP(v) kernel_tex_fetch(_ies, ofs + h * v_num + (v))
...
ccl_device_inline float kernel_ies_interp(inout KernelGlobals kg,
                                          int slot,
                                          float h_angle,
                                          float v_angle)
{
  /* Find offset of the IES data in_rsv the table. */
  int ofs = _float_as_int(kernel_tex_fetch(_ies, slot));
  if (ofs == -1) {
    return 100.0f;
  }

  int h_num = _float_as_int(kernel_tex_fetch(_ies, ofs++));
  int v_num = _float_as_int(kernel_tex_fetch(_ies, ofs++));

#define IES_LOOKUP_ANGLE_H(h) kernel_tex_fetch(_ies, ofs + (h))
#define IES_LOOKUP_ANGLE_V(v) kernel_tex_fetch(_ies, ofs + h_num + (v))
...
#undef IES_LOOKUP_ANGLE_V
  ...
  /* Cubic interpolation can result in_rsv negative values, so get rid of them. */
...
, uint4 node, inout int offset)
{
  ...
  float fac = strength * kernel_ies_interp(kg, int(slot), float(h_angle), float(v_angle));
