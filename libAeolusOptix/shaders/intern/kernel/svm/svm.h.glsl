...
ccl_device_inline float3 stack_load_float3(inout float stack[SVM_STACK_SIZE]
, uint a)
{
...
ccl_device_inline void stack_store_float3(inout float stack[SVM_STACK_SIZE]
, uint a, float3 f)
{
  kernel_assert(a + 2 < SVM_STACK_SIZE);
...
ccl_device_inline float stack_load_float(inout float stack[SVM_STACK_SIZE]
, uint a)
{
...
ccl_device_inline float stack_load_float_default(inout float stack[SVM_STACK_SIZE]
, uint a, uint value)
{
  return (a == uint(SVM_STACK_INVALID)) ? _uint_as_float(value) : stack_load_float(stack, a);

}

ccl_device_inline void stack_store_float(inout float stack[SVM_STACK_SIZE]
, uint a, float f)
{
  kernel_assert(a < SVM_STACK_SIZE);
...
ccl_device_inline int stack_load_int(inout float stack[SVM_STACK_SIZE]
, uint a)
{
  kernel_assert(a < SVM_STACK_SIZE);

  return _float_as_int(stack[a]);
}

ccl_device_inline int stack_load_int_default(inout float stack[SVM_STACK_SIZE]
, uint a, uint value)
{
...
ccl_device_inline void stack_store_int(inout float stack[SVM_STACK_SIZE]
, uint a, int i)
{
  kernel_assert(a < SVM_STACK_SIZE);

  stack[a] = _int_as_float(i);
}
...
ccl_device_inline uint4 read_node(inout KernelGlobals kg, inout int offset)
{
...
ccl_device_inline float4 read_node_float(inout KernelGlobals kg, inout int offset)
{
  uint4 node = kernel_tex_fetch(_svm_nodes, offset);
  float4 f = make_float4(_uint_as_float(node.x),
                         _uint_as_float(node.y),
...
ccl_device_inline float4 fetch_node_float(inout KernelGlobals kg, int offset)
{
  uint4 node = kernel_tex_fetch(_svm_nodes, offset);
  return make_float4(_uint_as_float(node.x),
                     _uint_as_float(node.y),
                     _uint_as_float(node.z),
                     _uint_as_float(node.w));
}

ccl_device_forceinline void svm_unpack_node_uchar2(uint i, inout uint x, inout uint y)
{
  x = (i & 0xFF);
  y = ((i >> 8) & 0xFF);
}

ccl_device_forceinline void svm_unpack_node_uchar3(uint i, inout uint x, inout uint y, inout uint z)
{
  x = (i & 0xFF);
  y = ((i >> 8) & 0xFF);
  z = ((i >> 16) & 0xFF);
...
ccl_device_forceinline void svm_unpack_node_uchar4(uint i, inout uint x, inout uint y, inout uint z, inout uint w)
{
  x = (i & 0xFF);
  y = ((i >> 8) & 0xFF);
  z = ((i >> 16) & 0xFF);
...
ccl_device_noinline void svm_eval_nodes(inout KernelGlobals kg,
                                        inout ShaderData sd,
...
#  endif /* NODES_FEATURE(NODE_FEATURE_VOLUME) */
      ...
      case NODE_PARTICLE_INFO:
        ...
        break;
#  if defined(_HAIR_) && NODES_FEATURE(NODE_FEATURE_HAIR)
      case NODE_HAIR_INFO:
...
#  endif /* NODES_FEATURE(NODE_FEATURE_VOLUME) */
      case NODE_AOV_START:
        if (!svm_node_aov_check(state, ofs)) {
          return;
        }
        break;
      case NODE_AOV_COLOR:
        svm_node_aov_color(kg, sd, stack, node, ofs);
        break;
      case NODE_AOV_VALUE:
        svm_node_aov_value(kg, sd, stack, node, ofs);
        break;
