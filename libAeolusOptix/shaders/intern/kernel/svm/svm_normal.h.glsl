...
ccl_device void svm_node_normal(inout KernelGlobals kg,
                                inout ShaderData sd,
                                ...
                                inout int offset)
{
  ...
  float3 direction;
  direction.x = _uint_as_float(node1.x);
  direction.y = _uint_as_float(node1.y);
  direction.z = _uint_as_float(node1.z);
  ...
  if (stack_valid(out_dot_offset))
    stack_store_float(stack, out_dot_offset, dot3(direction, normalize(normal)));
}
