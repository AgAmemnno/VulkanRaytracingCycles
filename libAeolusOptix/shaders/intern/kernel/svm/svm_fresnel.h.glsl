...
ccl_device void svm_node_fresnel(
    inout ShaderData sd, inout float stack[SVM_STACK_SIZE]
, uint ior_offset, uint ior_value, uint node)
{
  ...
  float eta = (stack_valid(ior_offset)) ? stack_load_float(stack, ior_offset) :
                                          _uint_as_float(ior_value);
  float3 normal_in = stack_valid(normal_offset) ? stack_load_float3(stack, normal_offset) : sd.N;

  eta = fmaxf(eta, 1e-5f);
  eta = bool(sd.flag & SD_BACKFACING) ? 1.0f / eta : eta;

  float f = fresnel_dielectric_cos(dot3(sd.I, normal_in), eta);
...
ccl_device void svm_node_layer_weight(inout ShaderData sd, inout float stack[SVM_STACK_SIZE]
, uint4 node)
{
  ...
  if (type == NODE_LAYER_WEIGHT_FRESNEL) {
    ...
    eta = bool(sd.flag & SD_BACKFACING) ? eta : 1.0f / eta;

    f = fresnel_dielectric_cos(dot3(sd.I, normal_in), eta);
  }
  else {
    f = fabsf(dot3(sd.I, normal_in));
