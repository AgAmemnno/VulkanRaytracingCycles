...
/* Orthographic Camera */
ccl_device void camera_sample_orthographic(float raster_x,
                                           float raster_y,
                                           float lens_u,
                                           float lens_v,
                                           ccl_addr_space inout Ray ray)
                                     ...
                                     ccl_addr_space inout Ray ray)
{
  ...
  float raster_x = x + lookup_table_read(filter_u, filter_table_offset, FILTER_TABLE_SIZE);
  float raster_y = y + lookup_table_read(filter_v, filter_table_offset, FILTER_TABLE_SIZE);
...
ccl_device_inline float3 camera_position(inout KernelGlobals kg)
{
  Transform cameratoworld = kernel_data.cam.cameratoworld;
  return make_float3(cameratoworld.x.w, cameratoworld.y.w, cameratoworld.z.w);
}

ccl_device_inline float camera_distance(inout KernelGlobals kg, float3 P)
{
  Transform cameratoworld = kernel_data.cam.cameratoworld;
...
ccl_device_inline float camera_z_depth(inout KernelGlobals kg, float3 P)
{
...
ccl_device_inline float3 camera_direction_from_point(inout KernelGlobals kg, float3 P)
{
  Transform cameratoworld = kernel_data.cam.cameratoworld;
...
ccl_device_inline float3 camera_world_to_ndc(inout KernelGlobals kg, inout ShaderData sd, float3 P)
{
  if (kernel_data.cam.type != CAMERA_PANORAMA) {
    /* perspective / ortho */
