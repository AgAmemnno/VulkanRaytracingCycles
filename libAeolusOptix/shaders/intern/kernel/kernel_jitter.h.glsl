#ifndef _KERNEL_JITTER_H_
#define _KERNEL_JITTER_H_
/*
...
{
  kernel_assert("assert jitter 139", (s < N));
...
{
#  if defined(_KERNEL_CUDA_)
  return float_to_int(_fsqrt_ru(value));
#  elif defined(_KERNEL_GPU_)
  return float_to_int(sqrtf(float(value)));
#  else
...
ccl_device void cmj_sample_2D(int s, int N, int p, inout float fx,inout  float fy)
{
  kernel_assert("assert jitter 165", (s < N));
  ...
  s = int(cmj_permute(s, N, p * 0x51633e2d));
...
ccl_device float pmj_sample_1D(int sample_rsv, int rng_hash, int dimension)
#else
ccl_device float pmj_sample_1D(KernelGlobals *kg, int sample_rsv, int rng_hash, int dimension)
#endif
{
  /* Fallback to random */
  if (sample_rsv >= NUM_PMJ_SAMPLES) {
    const int p = rng_hash + dimension;
    return cmj_randfloat(sample_rsv, p);
  }
  else {
    const uint mask_rsv = cmj_hash_simple(dimension, rng_hash) & uint(0x007fffff);
    const int index = ((dimension % NUM_PMJ_PATTERNS) * NUM_PMJ_SAMPLES + sample_rsv) * 2;
    return _uint_as_float(kernel_tex_fetch(_sample_pattern_lut, index) ^ mask_rsv) - 1.0f;
  }
}

#ifdef _KERNEL_VULKAN_
ccl_device float2 pmj_sample_2D(int sample_rsv, int rng_hash, int dimension)
#else
ccl_device float2 pmj_sample_2D(KernelGlobals *kg, int sample_rsv, int rng_hash, int dimension)
#endif
{
  if (sample_rsv >= NUM_PMJ_SAMPLES) {
    const int p = rng_hash + dimension;
    const float fx = cmj_randfloat(sample_rsv, p);
    const float fy = cmj_randfloat(sample_rsv, p + 1);
    ...
    const float fx = _uint_as_float(kernel_tex_fetch(_sample_pattern_lut, index) ^ maskx) - 1.0f;
    const float fy = _uint_as_float(kernel_tex_fetch(_sample_pattern_lut, index + 1) ^ masky) - 1.0f;
    return make_float2(fx, fy);
  }
}
