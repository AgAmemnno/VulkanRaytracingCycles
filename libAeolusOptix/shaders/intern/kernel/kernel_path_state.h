...
{
  state->flag = int(PATH_RAY_CAMERA | PATH_RAY_MIS_SKIP | PATH_RAY_TRANSPARENT_BACKGROUND);

  state->rng_hash = rng_hash;
  state->rng_offset = int(PRNG_BASE_NUM);
  state->sample = sample;
  ...
  if (kernel_data.film.pass_denoising_data !=0) {
    state->flag |= int(PATH_RAY_STORE_SHADOW_INFO);
...
ccl_device_inline void path_state_init(KernelGlobals *kg,
                                       inout ShaderDataTinyStorage stack_sd,
                                       ccl_addr_space PathState *state,
                                       uint rng_hash,
                                       int sample,
                                       ccl_addr_space Ray *ray)
{
  ...
  if (kernel_data.integrator.use_volumes !=0) {
    /* Initialize volume stack with volume we are inside of. */
    kernel_volume_stack_init(kg, stack_sd, state, ray, state->volume_stack);
  }
  else {
                                       ...
                                       int label)
{
  ...
  if (label & LABEL_VOLUME_SCATTER) {
    /* volume scatter */
    state->flag |= int(PATH_RAY_VOLUME_SCATTER);
...
#ifdef __DENOISING_FEATURES__
  if ((state->denoising_feature_weight == 0.0f) && !bool(state->flag & PATH_RAY_SHADOW_CATCHER)) {
    state->flag &= int(~PATH_RAY_STORE_SHADOW_INFO);
  }
#endif
}
...
ccl_device_inline uint path_state_ray_visibility(KernelGlobals *kg,
                                                 ccl_addr_space PathState *state)
{
  uint flag = uint(state->flag & int(PATH_RAY_ALL_VISIBILITY));

  /* for visibility, diffuse/glossy are for reflection only */
  if (bool(flag & PATH_RAY_TRANSMIT))
    flag &= ~(PATH_RAY_DIFFUSE | PATH_RAY_GLOSSY);
  /* todo: this is not supported as its own ray visibility yet */
  if (bool(state->flag & PATH_RAY_VOLUME_SCATTER))
...
{
  if (bool(state->flag & PATH_RAY_TERMINATE_IMMEDIATE)) {
    /* Ray is to be terminated immediately. */
    return 0.0f;
  }
  else if (bool(state->flag & PATH_RAY_TRANSPARENT) ){
    ...
    else if ( bool(state->flag & PATH_RAY_SHADOW_CATCHER) && (state->transparent_bounce <= 8)) {
      return 1.0f;
    }
#endif
  }
...
#ifdef __SHADOW_TRICKS__
    /* Exception for shadow catcher not working correctly with RR. */
    else if ( bool(state->flag & PATH_RAY_SHADOW_CATCHER) && (state->bounce <= 3)) {
      return 1.0f;
    }
#endif
  }
...
ccl_device_inline bool path_state_ao_bounce(KernelGlobals *kg, ccl_addr_space PathState *state)
{
  ...
  int bounce = state->bounce - state->transmission_bounce - int(state->glossy_bounce > 0);
