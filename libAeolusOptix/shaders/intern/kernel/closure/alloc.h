 ...
 */
#ifndef __CLOSURE_ALLOC_H__
#define __CLOSURE_ALLOC_H__

CCL_NAMESPACE_BEGIN

//ShaderClosure accessor
#define SDSC(n) sd.closure[n]

ccl_device int closure_alloc(inout ShaderData sd, int size, ClosureType type, float3 weight)
{
  kernel_assert(size <= sizeof_ShaderClosure);

  if (sd.num_closure_left == 0)
    return -1;


  sd.closure[sd.num_closure].type = type;
