...
#else
ccl_device_inline int bssrdf_alloc(ShaderData *sd, float3 weight)
#endif
{
  #ifndef __KERNEL_VULKAN__
  ...
  #else
  int n = closure_alloc(sd, sizeof_Bssrdf, CLOSURE_NONE_ID, weight);
  ...
  float sample_weight = fabsf(average(weight));
  sd.closure[n].sample_weight = sample_weight;
...
ccl_device int bssrdf_setup(ShaderData *sd, Bssrdf *bssrdf, ClosureType type)
{
  ...
  if (bssrdf_channels < 3) {
    ...
    if (type == CLOSURE_BSSRDF_PRINCIPLED_ID || type == CLOSURE_BSSRDF_PRINCIPLED_RANDOM_WALK_ID) {
      ...
      int n = bsdf_alloc(
          sd, sizeof_PrincipledDiffuseBsdf, diffuse_weight);

      if (n >= 0) {
        sd.closure[n].type = CLOSURE_BSDF_BSSRDF_PRINCIPLED_ID;
        sd.closure[n].N = N;
        PrincipledDiffuse_roughness(sd.closure[n] )= roughness;
        flag |= bsdf_principled_diffuse_setup(sd.closure[n]);
      }
    }
    else
#endif /* _PRINCIPLED_ */
    {
      int n = bsdf_alloc(sd, sizeof_DiffuseBsdf, diffuse_weight);

      if (n >= 0) {
        
       sd.closure[n].type = CLOSURE_BSDF_BSSRDF_ID;
       sd.closure[n].N = bssrdf.N;
       flag |= bsdf_diffuse_setup(sd.closure[n]);
