                                                             ...
                                                             const float3 cspec0)
{
  ...
  const float3 wh = normalize3(wi + wo);
...
//float v = beta(-lambda_r, shadowing_lambda + 1.0f);    
//PROFI_DATA_012(v,-lambda_r, shadowing_lambda + 1.0f)
//float _x =  -1. + 2.*float(PROFI_IDX)/12544.f;
//float v1 = float(polevl(_x,B, 5)/p1evl(_x,C, 6));
//float v2 = float(lgammaf(_x));
//PROFI_DATA_678(_x ,v1, v2);
  ...
  for (int order = 0; order < 10; order++) {
    ...
    if (order + 1 < 10) {
      ...
      wr = mf_sample_phase_glossy(-wr,throughput,
 wm);
#endif
                                                               ...
                                                               const float3 cspec0)
{
  ...
  float F0 = fresnel_dielectric_cos(1.0f, eta);
  if (use_fresnel) {
    throughput = interpolate_fresnel_color(wi, normalize3(wi + wr), eta, F0, cspec0);
  }
