...
#ifdef _OSL_
#include "kernel/closure/bsdf_phong_ramp.h.glsl"
#include "kernel/closure/bsdf_diffuse_ramp.h.glsl"
#endif
...
#include "kernel/closure/bsdf_hair.h.glsl"
#include "kernel/closure/bsdf_hair_principled.h.glsl"
...
#include "kernel/closure/volume.h.glsl"
// clang-format on
...
ccl_device_inline float bsdf_get_roughness_squared(in ShaderClosure sc)
{
  ...
  if (sc.type == CLOSURE_BSDF_OREN_NAYAR_ID) {
    return sqr(sqr(OrenNayar_roughness(sc)));
  }

  if (sc.type == CLOSURE_BSDF_PRINCIPLED_DIFFUSE_ID) {
    return sqr(sqr(PrincipledDiffuse_roughness(sc)));
  }
...
ccl_device_inline int bsdf_sample(inout KernelGlobals kg,
                                  inout ShaderData sd,
                                  in ShaderClosure sc,
                                  float randu,
                                  float randv,
                                  ...
                                  inout float pdf)
{
  /* For curves use the smooth_rsv normal, particularly for ribbons the geometric
   * normal gives too much darkening otherwise. */
  int label;
  const float3 Ng = (bool(sd.type & PRIMITIVE_ALL_CURVE)
) ? sc.N : sd.Ng;
#define CLOSURE_CONST
#include "kernel/closure/case_const.glsl"
...
,
    ...
    case CLOSURE_BSDF_HAIR_PRINCIPLED_ID:
      label = bsdf_principled_hair_sample(
          kg, sc, sd, randu, randv, eval, omega_in, domega_in.dx
, domega_in.dy
, pdf);
      break;
#  ifdef _PRINCIPLED_
...
#undef CLOSURE_CONST
  ...
  /* Test if BSDF sample_rsv should be treated as transparent for background. */
  if (bool(label & LABEL_TRANSMIT)
    ...
    const float frequency_multiplier =
        kernel_tex_fetch(_objects, sd.object).shadow_terminator_offset;
    ...
    if (bool(label & LABEL_DIFFUSE)
      ...
      if (!isequal_float3(sc.N, sd.N)) {
        eval *= bump_shadowing_term((bool(label & LABEL_TRANSMIT)
) ? -sd.N : sd.N, sc.N, omega_in);
      }
    }
  }
...
#endif
    float3
    bsdf_eval(inout KernelGlobals kg,
              inout ShaderData sd,
              in ShaderClosure sc,
...
ccl_device void bsdf_blur(inout KernelGlobals kg, inout ShaderClosure sc, float roughness)
{
...
ccl_device bool bsdf_merge(inout ShaderClosure a, inout ShaderClosure b)
{
#ifdef _SVM_
#define CLOSURE_CONST
#include "kernel/closure/case_const.glsl"
