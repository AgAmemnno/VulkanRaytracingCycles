...
ccl_device float pmj_sample_1D(in KernelGlobals kg,int sample, int rng_hash, int dimension)
#else
...
#ifdef __KERNEL_VULKAN__
ccl_device float2 pmj_sample_2D(in KernelGlobals kg,int sample, int rng_hash, int dimension)
#else
...
#endif
{
  if (sample >= NUM_PMJ_SAMPLES) {
    ...
    const float fy = __uint_as_float(kernel_tex_fetch(__sample_pattern_lut, index + 1) ^ masky) - 1.0f;
    return make_float2(fx, fy);
  }
}
