#ifndef _KERNEL_LIGHT_H_
#define _KERNEL_LIGHT_H_
/*
...
#include "kernel_light_background.h.glsl"
...
ccl_device_forceinline float triangle_light_pdf(inout ShaderData sd, float t)
{
  ...
  const float distance_to_plane = fabsf(dot3(N, sd.I * t)) / dot3(N, N);
  ...
  if (longest_edge_squared > distance_to_plane * distance_to_plane) {
    ...
    const float alpha = fast_acosf(dot3(u02, u01));
    const float beta = fast_acosf(-dot3(u01, u12));
    const float gamma = fast_acosf(dot3(u02, u12));
...
void triangle_light_sample(int prim,
                          int object,
                          float randu,
                          float randv,
                          float time,
                          ...
                          const float3 P)
{
  ...
  if (longest_edge_squared > distance_to_plane * distance_to_plane) {
    ...
    const float3 u01 = safe_normalize(cross3(v0_p, v1_p));
    const float3 u02 = safe_normalize(cross3(v0_p, v2_p));
    const float3 u12 = safe_normalize(cross3(v1_p, v2_p));

    const float3 A = safe_normalize(v0_p);
    const float3 B = safe_normalize(v1_p);
    const float3 C = safe_normalize(v2_p);

    const float cos_alpha = dot3(u02, u01);
    const float cos_beta = -dot3(u01, u12);
    const float cos_gamma = dot3(u02, u12);
    ...
    const float3 U = safe_normalize(C - dot3(C, A) * A);
    float q = 1.0f;
    ...
    const float z = 1.0f - randv * (1.0f - dot3(C_, B));
    ls.D = z * B + safe_sqrtf(1.0f - z * z) * safe_normalize(C_ - dot3(C_, B) * B);
...
if (type == LIGHT_DISTANT) {
    ...
    if (type == LIGHT_POINT || type == LIGHT_SPOT) {
      ...
      ls.u = dot3(inplane, axisu) * (1.0f / dot3(axisu, axisu)) + 0.5f;
      ls.v = dot3(inplane, axisv) * (1.0f / dot3(axisv, axisv)) + 0.5f;
...
= radius*radius*pi
  ...
  if (type == LIGHT_SPOT) {
    ...
    /* one sided */
    if (dot3(D, Ng) >= 0.0f) return false;
