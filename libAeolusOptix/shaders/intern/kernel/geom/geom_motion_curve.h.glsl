...
ccl_device_inline int find_attribute_curve_motion(inout KernelGlobals kg,
                                                  int object,
                                                  uint id,
                                                  inout AttributeElement elem)
{
   ...
   *
   * NOTE: currently it's not a bottleneck because in_rsv test scenes the loop below runs
...
ccl_device_inline void motion_curve_keys_for_step_linear(inout KernelGlobals kg,
                                                         int offset,
                                                         int numkeys,
                                                         int numsteps,
                                                         int step,
...
2])
{
  if (step == numsteps) {
    ...
    keys[0] = kernel_tex_fetch(_curve_keys, k0);
    keys[1] = kernel_tex_fetch(_curve_keys, k1);
  }
  else {
    ...
    keys[0] = kernel_tex_fetch(_attributes_float3, offset + k0);
    keys[1] = kernel_tex_fetch(_attributes_float3, offset + k1);
  }
}

/* return 2 curve key locations */
ccl_device_inline void motion_curve_keys_linear(
    inout KernelGlobals kg, int object, int prim, float time, int k0, int k1, inout float4 keys[
2])
{
  /* get motion info */
  ...
  object_motion_info(kg, object, (numsteps), null_int, (numkeys));
...
ccl_device_inline void motion_curve_keys_for_step(inout KernelGlobals kg,
                                                  int offset,
                                                  int numkeys,
                                                  int numsteps,
                                                  int step,
...
4])
{
  if (step == numsteps) {
    ...
    keys[0] = kernel_tex_fetch(_curve_keys, k0);
    keys[1] = kernel_tex_fetch(_curve_keys, k1);
    keys[2] = kernel_tex_fetch(_curve_keys, k2);
    keys[3] = kernel_tex_fetch(_curve_keys, k3);
  }
  else {
    /* center step is not stored in_rsv this array */
    if (step > numsteps)
    ...
    keys[0] = kernel_tex_fetch(_attributes_float3, offset + k0);
    keys[1] = kernel_tex_fetch(_attributes_float3, offset + k1);
    keys[2] = kernel_tex_fetch(_attributes_float3, offset + k2);
    keys[3] = kernel_tex_fetch(_attributes_float3, offset + k3);
  }
}

/* return 2 curve key locations */
ccl_device_inline void motion_curve_keys(inout KernelGlobals kg,
                                         int object,
                                         int prim,
                                         float time,
                                         int k0,
