...
#ifndef __GEOM_TRI_H__
#define __GEOM_TRI_H__
CCL_NAMESPACE_BEGIN
...
ccl_device_inline float3 triangle_normal(KernelGlobals *kg, ShaderData *sd)
{
  ...
  if (bool(sd->object_flag & SD_OBJECT_NEGATIVE_SCALE_APPLIED) ){
    return make_float3(normalize(cross((v2 - v0).xyz, (v1 - v0).xyz)));
     
  }
  else {
    return make_float3(normalize(cross((v1- v0).xyz, (v1 - v0).xyz)));
  }
}
    ...
    KernelGlobals *kg, int object, int prim, float u, float v,float3* P,float3* Ng,int* shader)
{
  ...
  P = (u * v0 + v * v1 + t * v2);
  /* get object flags */
  int object_flag = int(kernel_tex_fetch(__object_flag, object));
  /* compute normal */
  if(bool(object_flag & SD_OBJECT_NEGATIVE_SCALE_APPLIED) ){
    Ng = make_float3(normalize(cross((v2 - v0).xyz, (v1 - v0).xyz )));
  }
  else {
    Ng = make_float3(normalize(cross((v1 - v0).xyz, (v2 - v0).xyz )));
  }

  /* shader`*/
  shader = int(kernel_tex_fetch(__tri_shader, prim));
}
...
#ifdef __RAY_DIFFERENTIALS__
    if (dx2)
      *dx2 = sd->du.dx * f0 + sd->dv.dx * f1 - (sd->du.dx + sd->dv.dx) * f2;
    if (dy2)
      *dy2 = sd->du.dy * f0 + sd->dv.dy * f1 - (sd->du.dy + sd->dv.dy) * f2;
#endif
...
#ifdef __RAY_DIFFERENTIALS__
     if (dx3)
       dx3 = sd->du.dx * f0 + sd->dv.dx * f1 - (sd->du.dx + sd->dv.dx) * f2;
     if (dy3)
       dy3 = sd->du.dy * f0 + sd->dv.dy * f1 - (sd->du.dy + sd->dv.dy) * f2;
#endif
...
#ifdef __RAY_DIFFERENTIALS__
    if (dx4)
      *dx4 = sd->du.dx * f0 + sd->dv.dx * f1 - (sd->du.dx + sd->dv.dx) * f2;
    if (dy4)
      *dy4 = sd->du.dy * f0 + sd->dv.dy * f1 - (sd->du.dy + sd->dv.dy) * f2;
#endif
