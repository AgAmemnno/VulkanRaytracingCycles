...
ccl_device_noinline void motion_triangle_shader_setup(
    inout KernelGlobals kg, inout ShaderData sd, in Intersection isect, in Ray ray, bool is_local)
{
  /* Get shader. */
  ...
  int numsteps, numverts;
    object_motion_info(kg, sd.object, (numsteps), (numverts), null_int);



  /* Figure out_rsv which steps we need to fetch and their interpolation factor. */
  int maxstep = numsteps * 2;
  int step = min(int(sd.time * maxstep)
, maxstep - 1);
  float t = sd.time * maxstep - step;
  /* Find attribute. */
  AttributeElement elem;
  int offset = find_attribute_motion(kg, int(sd.object), uint(ATTR_STD_MOTION_VERTEX_POSITION), (elem));


  kernel_assert(offset != ATTR_STD_NOT_FOUND);
  /* Fetch vertex coordinates. */
  float3 verts[3], next_verts[3];
  uint4 tri_vindex = kernel_tex_fetch(_tri_vindex, sd.prim);
