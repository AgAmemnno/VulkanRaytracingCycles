 ...
 */
#ifndef _GEOM_MOTION_TRIANGLE_H_
#define _GEOM_MOTION_TRIANGLE_H_
CCL_NAMESPACE_BEGIN
...
ccl_device_inline void motion_triangle_normals_for_step(
                                                        uint4 tri_vindex,
                                                        int offset,
                                                        int numverts,
                                                        int numsteps,
                                                        int step,
                                                        inout float3 normals[3])

{
  if (step == numsteps) {
    /* center step: regular vertex location */
    normals[0] = float4_to_float3(kernel_tex_fetch(_tri_vnormal, tri_vindex.x));
    normals[1] = float4_to_float3(kernel_tex_fetch(_tri_vnormal, tri_vindex.y));
    normals[2] = float4_to_float3(kernel_tex_fetch(_tri_vnormal, tri_vindex.z));
  }
  else {
    /* center step is not stored in_rsv this array */
    if (step > numsteps)
      step--;

    offset += step * numverts;

    normals[0] = float4_to_float3(kernel_tex_fetch(_attributes_float3, offset + tri_vindex.x));
    normals[1] = float4_to_float3(kernel_tex_fetch(_attributes_float3, offset + tri_vindex.y));
    normals[2] = float4_to_float3(kernel_tex_fetch(_attributes_float3, offset + tri_vindex.z));
  }
}
...
ccl_device_inline float3 motion_triangle_smooth_normal(float3 Ng, int object, int prim, float u, float v, float time)
{
  /* get motion info */
  int numsteps, numverts;
  {int null_int = 0; object_motion_info(object, numsteps, numverts, null_int);}


  /* figure out_rsv which steps we need to fetch and their interpolation factor */
  int maxstep = numsteps * 2;
  int step = min(int(time * maxstep)
, maxstep - 1);
  float t = time * maxstep - step;

  /* find attribute */
  AttributeElement elem;
  int offset = find_attribute_motion(object, ATTR_STD_MOTION_VERTEX_NORMAL, (elem));

  kernel_assert("assert  GEOM_MOTION_TRI : 159 ",offset != ATTR_STD_NOT_FOUND);

  /* fetch normals */
  float3 normals[3], next_normals[3];
  uint4 tri_vindex = kernel_tex_fetch(_tri_vindex, prim);

  motion_triangle_normals_for_step(tri_vindex, offset, numverts, numsteps, step, normals);
  motion_triangle_normals_for_step(tri_vindex, offset, numverts, numsteps, step + 1, next_normals);
