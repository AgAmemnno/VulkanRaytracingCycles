...
patch_map_find_patch(KernelGlobals *kg, int object, int patch, float u, float v)
{
  ...
  int node = int((object_patch_map_offset(kg, object) + patch) / 2);
  float median = 0.5f;
  ...
  for (int depth = 0; depth < 0xff; depth++) {
    ...
    if (child & PATCH_MAP_NODE_IS_LEAF) {
      handle.array_index = int(kernel_tex_fetch(__patches, index + 0));
      handle.patch_index = int(kernel_tex_fetch(__patches, index + 1));
      handle.vert_index  = int(kernel_tex_fetch(__patches, index + 2));
