...
ccl_device uint sobol_dimension(in KernelGlobals kg,int index, int dimension)
#else
...
ccl_device_forceinline float path_rng_1D(
    in KernelGlobals kg, uint rng_hash, int sample, int num_samples, int dimension)
#else
ccl_device_forceinline float path_rng_1D(
...
#  endif
  {
    ...
    return cmj_sample_1D(kg,sample, num_samples, p);
  }
#endif
...
#ifdef __KERNEL_VULKAN__
ccl_device_forceinline void path_rng_2D(in KernelGlobals kg,
                                        uint rng_hash,
                                        int sample,
                                        int num_samples,
                                        int dimension,
...
#ifdef __KERNEL_VULKAN__
ccl_device_inline void path_rng_init(in KernelGlobals kg,
                                     int sample,
                                     int num_samples,
                                     inout uint rng_hash,
                                     int x,
...
ccl_device uint lcg_init(inout uint seed)
{
  uint rng = seed;
  lcg_step_uint(rng);
  return rng;
...
ccl_device_inline float path_state_rng_1D(in KernelGlobals kg,
                                          const ccl_addr_space PathState *state,
                                          int dimension)
{
  return path_rng_1D(
