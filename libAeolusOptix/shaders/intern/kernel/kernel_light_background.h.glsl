#ifndef _KERNEL_LIGHT_BG_H_
#define _KERNEL_LIGHT_BG_H_
/*
...
#include "kernel_light_common.h.glsl"
#include "kernel_projection.h.glsl"
CCL_NAMESPACE_BEGIN
...
ccl_device float3 background_map_sample(float randu, float randv, inout float pdf)
{
  ...
  while (count > 0) {
    ...
    if (kernel_tex_fetch(_light_background_marginal_cdf, middle).y < randv) {
      first = middle + 1;
      count -= step + 1;
    }
    else
  ...
  kernel_assert("assert  kernel_light bg :: 51 ",index_v >= 0 && index_v < res_y);

  float2 cdf_v = kernel_tex_fetch(_light_background_marginal_cdf, index_v);
  float2 cdf_next_v = kernel_tex_fetch(_light_background_marginal_cdf, index_v + 1);
  float2 cdf_last_v = kernel_tex_fetch(_light_background_marginal_cdf, res_y);
  ...
  while (count > 0) {
    ...
    if (kernel_tex_fetch(_light_background_conditional_cdf, index_v * cdf_width + middle).y <
        randu) {
      first = middle + 1;
      count -= step + 1;
    }
  ...
  kernel_assert("assert  kernel_light bg :: 78 ",index_u >= 0 && index_u < res_x);

  float2 cdf_u = kernel_tex_fetch(_light_background_conditional_cdf,
                                  index_v * cdf_width + index_u);
  float2 cdf_next_u = kernel_tex_fetch(_light_background_conditional_cdf,
                                       index_v * cdf_width + index_u + 1);
  float2 cdf_last_u = kernel_tex_fetch(_light_background_conditional_cdf,
 ...
 */
ccl_device float background_map_pdf(float3 direction)
{
...
ccl_device_inline bool background_portal_data_fetch_and_check_side(float3 P, int index, inout float3 lightpos, inout float3 dir)
{
  int portal = kernel_data.background.portal_offset + index;
  const ccl_global KernelLight klight = 
kernel_tex_fetch(_lights, portal);

  lightpos = make_float3(klight.co[0], klight.co[1], klight.co[2]);
  dir = make_float3(AreaLight_dir0(klight)
, AreaLight_dir1(klight)
, AreaLight_dir2(klight)
);

  /* Check whether portal is on the right side. */
  if (dot3(dir, (P - lightpos)) > 1e-4f)
    return true;

  return false;
}

ccl_device_inline float background_portal_pdf( float3 P, float3 direction, int ignore_portal, inout bool is_possible)
{
  ...
  for (int p = 0; p < kernel_data.background.num_portals; p++) {
    ...
    float3 lightpos, dir;
    if (!background_portal_data_fetch_and_check_side(P, p, lightpos, dir
))
      continue;
...
kernel_tex_fetch(_lights, portal);
    float3 axisu = make_float3(
        AreaLight_axisu0(klight)
, AreaLight_axisu1(klight)
, AreaLight_axisu2(klight)
);
    float3 axisv = make_float3(
        AreaLight_axisv0(klight) 
, AreaLight_axisv1(klight) 
, AreaLight_axisv2(klight) 
);
    bool is_round = (AreaLight_invarea(klight) 
 < 0.0f);
    ...
    if (is_round) {
      ...
      portal_pdf += fabsf(AreaLight_invarea(klight) ) * lamp_light_pdf(dir, -D, t);
    }
    else {
...
ccl_device int background_num_possible_portals(float3 P)
{
...
ccl_device float3 background_portal_sample(
                                           float3 P,
                                           float randu,
                                           float randv,
                                           int num_possible,
                                           inout int sampled_portal,
                                           inout float pdf)
{
...
randv -= portal;
  ...
  /* TODO(sergey): Some smarter way of finding portal to sample_rsv
   * is welcome.
   */
  for (int p = 0; p < kernel_data.background.num_portals; p++) {
    ...
    if (portal == 0) {
      ...
      int portal = kernel_data.background.portal_offset + p;
      const ccl_global KernelLight klight = kernel_tex_fetch(_lights, portal);
      ...
      if (is_round) {
        ...
        pdf = fabsf(AreaLight_invarea(klight) ) * lamp_light_pdf(dir, -D, t);
      }
      else {
...
ccl_device_inline float3 background_sun_sample(
                                               float randu,
                                               float randv,
                                               inout float pdf)
{
...
ccl_device_inline float background_sun_pdf(float3 D)
{
  const float3 N = float4_to_float3(kernel_data.background.sun);
  const float angle = kernel_data.background.sun.w;
...
ccl_device_inline float3 background_light_sample(float3 P, float randu, float randv, inout float pdf)
{
  ...
  if (portal_method_pdf > 0.0f) {
    ...
    num_portals = background_num_possible_portals( P);
    if (num_portals == 0) {
  ...
  if (randu < portal_method_pdf) {
    ...
    /* Sample a portal. */
    int portal;
    D = background_portal_sample(P, randu, randv, num_portals, portal, pdf);
    if (num_portals > 1) {
      /* Ignore the chosen portal, its pdf is already included. */
      #ifdef _KERNEL_VULKAN_
      bool null_boo = false;
       pdf += background_portal_pdf(P, D, portal, null_boo);
      #else
      pdf += background_portal_pdf(P, D, portal, NULL);
      #endif
    }
    ...
    D = background_sun_sample(randu, randv, pdf);
    ...
    D = background_map_sample(randu, randv, pdf);
...
#ifdef _KERNEL_VULKAN_
bool null_boo = false;
pdf += portal_method_pdf * background_portal_pdf(P, D, -1,null_boo);
#else
pdf += portal_method_pdf * background_portal_pdf(P, D, -1, NULL);
#endif

  }
  if (method != 1 && sun_method_pdf != 0.0f) {
    pdf += sun_method_pdf * background_sun_pdf(D);
  }
  if (method != 2 && map_method_pdf != 0.0f) {
    pdf += map_method_pdf * background_map_pdf(D);
  }
  return D;
}

ccl_device float background_light_pdf(float3 P, float3 direction)
{
  float portal_method_pdf = kernel_data.background.portal_weight;
  float sun_method_pdf = kernel_data.background.sun_weight;
  float map_method_pdf = kernel_data.background.map_weight;
   ...
   * to find out_rsv if we can sample_rsv them. */
  if (portal_method_pdf > 0.0f) {
    /* Evaluate PDF of sampling this direction by portal sampling. */
    bool is_possible = false;
    portal_pdf = background_portal_pdf(P, direction, -1, is_possible
);
    if (!is_possible) {
  ...
  if (sun_method_pdf != 0.0f) {
    pdf += background_sun_pdf(direction) * sun_method_pdf;
  }
  if (map_method_pdf != 0.0f) {
    pdf += background_map_pdf(direction) * map_method_pdf;
  }
