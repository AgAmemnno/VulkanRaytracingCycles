...
#include "kernel/kernel_write_passes.h.glsl"
...
#ifdef GLAD
ccl_device_inline void kernel_write_result(int buffer_ofs,int sample_rsv)
{
...
#ifdef GSTATE
ccl_device_inline void kernel_write_data_passes()
{
#ifdef _PASSES_

  int path_flag = GSTATE.flag;
  ...
  if (!bool((flag | light_flag) & PASS_ANY))
    return;
...
if (!bool(path_flag & PATH_RAY_SINGLE_PASS_DONE)) {
     GSTATE.flag |= int(PATH_RAY_SINGLE_PASS_DONE);
}
#endif
#endif
}
