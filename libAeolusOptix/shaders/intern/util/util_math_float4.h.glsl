...
ccl_device_inline float4 sqr(const in float4 a)
{
  return a * a;
}
...
ccl_device_inline float4 reduce_add_float4(const in  float4 a)
{
...
#define select "NIL_SELECT"
#define mask "NIL_MASK" 
#define reduce_min "NIL_reduce_min"
#define reduce_max "NIL_reduce_max"
#define load_float4 "NIL_load_float4"
...
_forceinline const float4 shuffle(const float4 &b);
template<size_t index_0, size_t index_1, size_t index_2, size_t index_3>
_forceinline const float4 shuffle(const float4 &a, const float4 &b);

template<> _forceinline const float4 shuffle<0, 1, 0, 1>(const float4 &b);

template<> _forceinline const float4 shuffle<0, 1, 0, 1>(const float4 &a, const float4 &b);
template<> _forceinline const float4 shuffle<2, 3, 2, 3>(const float4 &a, const float4 &b);

#  ifdef _KERNEL_SSE3_
template<> _forceinline const float4 shuffle<0, 0, 2, 2>(const float4 &b);
template<> _forceinline const float4 shuffle<1, 1, 3, 3>(const float4 &b);
#  endif
#endif /* _KERNEL_SSE_ */
...
{
#  ifdef _KERNEL_SSE_
  _m128 mask = _mm_castsi128_ps(_mm_set1_epi32(0x80000000));
...
ccl_device_inline float dot(const float4 &a, const float4 &b)
{
#  if defined(_KERNEL_SSE41_) && defined(_KERNEL_SSE_)
...
#  else
  return (IS_FZERO(a.x) && IS_FZERO(a.y) && IS_FZERO(a.z) && IS_FZERO(a.w));
#  endif
}
