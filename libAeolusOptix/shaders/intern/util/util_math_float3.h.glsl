...
float len(vec3 a){return length(a);}
#define  len3(a) length(a.xyz)



ccl_device_inline float dot_xy(const in  float3 a, const in float3 b)
{
  return a.x * b.x + a.y * b.y;
}


ccl_device_inline float3 rcp(const in float3 a)
{
#ifndef FLOAT3_AS_VEC3
  if(a.w != 0.f){
...
ccl_device_inline float3 normalize_len(const float3 a, inout float t)
{
  /* pointer error */
  t = len3(a);
  float x = 1.0f / t;
  return make_float3_v3(a.xyz * x);
}
...
{
#  if defined(_KERNEL_SSE41_) && defined(_KERNEL_SSE_)
  _m128 norm = _mm_sqrt_ps(_mm_dp_ps(a.m128, a.m128, 0x7F));
...
{
#  ifdef _KERNEL_SSE_
  _m128 mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
...
ccl_device_inline float3 reflect(const float3 incident, const float3 normal)
{
  ...
  return incident - 2.0f * unit_normal * dot3(incident, unit_normal);
}
...
ccl_device_inline float3 project(const float3 v, const float3 v_proj)
{
  float len_squared = dot3(v_proj, v_proj);
  return (len_squared != 0.0f) ? (dot3(v, v_proj) / len_squared) * v_proj :
...
#else
  return (IS_FZERO(a.x) && IS_FZERO(a.y)&& IS_FZERO(a.z)  );
#endif
}
...
{
  #ifndef FLOAT3_AS_VEC3
  if(a.w != 0.f)return reduce_add(a) * 0.25f;
  #endif
  return reduce_add(a) * (1.0f / 3.0f);
}

#define isequal_float3(a,b) (a.xyz == b.xyz)
