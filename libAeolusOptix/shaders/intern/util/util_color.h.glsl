...
{
  return ((val <= 0.0f) ? uchar(0) : ((val > (1.0f - 0.5f / 255.0f)) ? uchar(255) : uchar((255.0f * val) + 0.5f)));
}
...
template<unsigned exp,
 uint e2coeff> ccl_device_inline ssef fastpow(const ssef &arg)
{
  ssef ret;
...
ccl_device float3 color_highlight_compress(float3 color, in float3 variance)
{
  color += make_float3(1.0f, 1.0f, 1.0f);
    ///NULL PTR
  if (variance != float3(0)) {
