...
ccl_device_inline bool is_zero(in float2 a)
{
  return (IS_FZERO(a.x) && IS_FZERO(a.y));
}

ccl_device_inline float average(in float2 a)
{
  return (a.x + a.y) * (1.0f / 2.0f);
}


ccl_device_inline float2 normalize_len(in float2 a,inout  float t)
{
  /* pointer error  */
  t = len(a);
  return a / (t);
...
ccl_device_inline float2 safe_normalize(in float2 a)
{
  float t = len(a);
  return (t != 0.0f) ? a / t : a;
}
...
ccl_device_inline float2 as_float2(in float4 a)
{
  return make_float2(a.x, a.y);
}


ccl_device_inline float2 safe_divide_float2_float(const in float2 a, const in float b)
{
