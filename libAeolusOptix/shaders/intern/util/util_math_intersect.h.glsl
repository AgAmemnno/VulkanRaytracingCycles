                                     ...
                                     out float  isect_t)
{
  ...
  const float tsq = dot3(d, d);

  if (tsq > radiussq) {
    /* Ray origin outside sphere. */
    const float tp = dot3(d, ray_D);
    if (tp < 0.0f) {
                                           ...
                                           out float isect_t)
{
  ...
  const float div = dot3(ray_D, disk_N);
                                                   ...
                                                   out float isect_t)
{
...
#else  /* _KERNEL_SSE2_ */
  const float U = dot3(cross3( (v2 + v0), e0), ray_dir);
  const float V = dot3(cross3( (v0 + v1), e1), ray_dir);
  const float W = dot3(cross3( (v1 + v2), e2), ray_dir);
#endif /* _KERNEL_SSE2_ */
  ...
  const int sign_den = (_float_as_int(den) & 0x80000000);
                                   ...
                                   bool ellipse)
{
  ...
  float t = -(dot3(ray_P, quad_n) - dot3(quad_P, quad_n)) / dot3(ray_D, quad_n);
  if (t < ray_mint || t > ray_maxt) {
    return false;
  }
  const float3 hit = ray_P + t * ray_D;
  const float3 inplane = hit - quad_P;
  const float u = dot3(inplane, quad_u) / dot3(quad_u, quad_u);
  if (u < -0.5f || u > 0.5f) {
    return false;
  }
  const float v = dot3(inplane, quad_v) / dot3(quad_v, quad_v);
  if (v < -0.5f || v > 0.5f) {
    return false;
  }
