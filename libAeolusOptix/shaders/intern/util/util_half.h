...
ccl_device_inline void float4_store_half(half *h, float4 f, float scale)
{
  ...
  #ifdef __KERNEL_VULKAN__
   _lerp(float16_t,float16_t)
   void float4_store_half(out half4 h,in float4 f, float scale){
            h = half4(f*scale);
   };
   #define  half_to_float(h) float(h)
   #define  half4_to_float4(h) vec4(h)
   #define  float_to_half(f) float16_t(f)
