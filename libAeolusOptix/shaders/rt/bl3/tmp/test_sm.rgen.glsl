...
{  
 ...
 uint Dy = gl_LaunchSizeNV.y;
  ...
  PROFI_HIT_IDX(float(atomic_offset),float(gl_SMIDNV),float(gl_WarpIDNV),float(gl_SubgroupInvocationID));//float(gl_PrimitiveIDIn) );
#endif

  if ( (gl_LaunchIDNV.x ==0)  &&  (gl_LaunchIDNV.y == 0) ){
    PROFI_IDX  = int(Dx*Dy);
    PROFI_HIT_IDX(float(gl_SMCountNV),float(gl_WarpsPerSMNV),float(gl_SubgroupSize),float(MAX_CLOSURE));//float(gl_PrimitiveIDIn) );
  }
