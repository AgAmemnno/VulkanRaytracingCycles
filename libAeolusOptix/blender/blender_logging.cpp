...
#include "blender_sync2.h"
#include "blender_session2.h"
#include "Python.h"
...
void BlenderSession::render(BL::Depsgraph& b_depsgraph_)
{
    ...
    session->debugStr = std::string(b_rlay_name);
...
void BlenderSession::reset_session(BL::BlendData& b_data, BL::Depsgraph& b_depsgraph)
{
   ...
   // PointerRNA propptr;
   // rna_lookup_string(&b_scene.ptr, "circus", &propptr);
    
    if (preview_osl) {
        PointerRNA cscene = RNA_pointer_get(&b_scene.ptr, "circus");
...
int main() {
    ...
    BL::RenderEngine b_engine(pRNA);
    BL::Preferences  b_userpref(pRNA);
    BL::BlendData  b_data(pRNA);

    BlenderSession BLsession( b_engine,
        b_userpref,
         b_data,
       false);

    BL::Depsgraph b_depsgrah(pRNA);
    BL::Object b_object(pRNA);

    const string& pass_type = "";
    const int custom_flag = 0;
    const int bake_width = 1024;
        const int bake_height = 1024;

    BLsession.bake(b_depsgrah, b_object, pass_type,
        custom_flag ,
        bake_width ,
        bake_height
        );
