                                           ...
                                           PathRadiance *L)
{
  ...
  if (kernel_data.film.pass_flag & PASSMASK(COMBINED)) {
     //L_sum = L->state.diffuse;
    kernel_write_pass_float4(buffer, make_float4(L_sum.x, L_sum.y, L_sum.z, alpha));
    
    if (stat_aux.write) {
        /*
        float3 V3 = L_sum; // ray->dP.dx;// 
        float prec = 1000.f;
        stat_aux.add(&stat_aux.data[5], int(prec * V3.x));
        stat_aux.add(&stat_aux.data[6], int(prec * V3.y));
        stat_aux.add(&stat_aux.data[7], int(prec * V3.z));
        */
    }
