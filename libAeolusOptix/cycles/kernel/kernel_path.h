...
#ifdef  WITH_STAT_ALL
        STAT_DUMP(indirect_lamp_emission_emission, L->emission);
        if (stat_aux.use_light_pass) {
            STAT_DUMP(indirect_lamp_emission_direct_emission, L->direct_emission);
            STAT_DUMP(indirect_lamp_emission_indirect, L->indirect);
        }
#endif
...
#ifdef  WITH_STAT_ALL
        STAT_DUMP(kernel_path_background_transparent, L->transparent);
#endif
...
#ifdef  WITH_STAT_ALL
    STAT_DUMP(kernel_path_background_L, L_background);
#endif
    ...
    ccl_global float* buffer)
{
...
#ifdef  WITH_STAT_ALL
    {
        ...
        STAT_DUMP(kernel_path_shader_apply_state_flag, flag);
    }
#endif
...
#ifdef  WITH_STAT_ALL
                stat_aux.add("kernel_path_shader_apply_bg",bg);
#endif
            }
...
#ifdef  WITH_STAT_ALL
        STAT_DUMP(kernel_path_shader_apply_emission_emission, L->emission);
        if (stat_aux.use_light_pass) {
            STAT_DUMP(kernel_path_shader_apply_emission_direct_emission, L->direct_emission);
            STAT_DUMP(kernel_path_shader_apply_emission_indirect, L->indirect);
        }
#endif
...
#ifdef  WITH_STAT_ALL
            if(stat_aux.rec_num ==0)
                if(!hit) stat_aux.addMiss();
                else CNT_ADD(CNT_HIT);

            if (hit) {
                auto object = (isect.object == OBJECT_NONE) ? kernel_tex_fetch(__prim_object, isect.prim) : isect.object;
                if (stat_aux.rec_num == 2) {
                    int i = 0;
                }
                stat_aux.objectAdd(object);
            }

            CNT_ADD((CNT_REC + stat_aux.rec_num)); stat_aux.rec_num++;
...
#ifdef  WITH_STAT_ALL
            {
                UINT bounce = UINT(state->bounce);
                STAT_DUMP(kernel_path_surface_bounce_state_bounce, bounce);
                STAT_DUMP(kernel_path_surface_bounce_rayD, ray->D);
            }
#endif
...
#  ifdef __SUBSURFACE__
        ...
        if (ss_indirect.num_rays) {
            ...
            STAT_DUMP(kernel_path_subsurface_setup_THR, throughput);
#endif
    ...
    KernelGlobals* kg, ccl_global float* buffer, int sample, int x, int y, int offset, int stride)
{
    ...
    stat_aux.start( (bool)kernel_data.film.use_light_pass);
    stat_aux.pixel_init(x, y,kg);
   // printf("pixel %d %d  \n", x, y);
#endif
