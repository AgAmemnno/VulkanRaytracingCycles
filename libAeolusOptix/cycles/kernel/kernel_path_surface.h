...
#    endif
      ...
      if (kernel_data.integrator.use_direct_light && (sd->flag & SD_BSDF_HAS_EVAL)) {
        ...
        if (light_sample(kg, lamp, light_u, light_v, sd->time, sd->P, state->bounce, &ls)) {
          ...
          STAT_DUMP(direct_emission_diffuse, L_light.diffuse);
          STAT_DUMP(direct_emission_sum_no_mis, L_light.sum_no_mis);
          if (stat_aux.use_light_pass) {
              STAT_DUMP(direct_emission_glossy, L_light.glossy);
              STAT_DUMP(direct_emission_transmission, L_light.transmission);
          }
#endif
...
#ifdef  WITH_STAT_ALL
      if (blocked) {
          CNT_ADD(CNT_shadow_blocked);
#ifdef shadow_blocked_ray_P
          STAT_DUMP(shadow_blocked_ray_P, light_ray.P);
#endif
      }
#endif
      if (has_emission) {
        ...
        STAT_DUMP_ADD(has_emission_nums);
        STAT_DUMP(has_emission_shadow, shadow);
        STAT_DUMP(has_emission_throughput, throughput);
#endif
                                           ...
                                           ccl_addr_space Ray *ray)
{
  ...
  {
      UINT falg = UINT(sd->flag);
      STAT_DUMP(kernel_path_surface_bounce_flag, falg);
  };
#endif
#endif
  ...
  if (sd->flag & SD_BSDF) {
      ...
      STAT_DUMP(kernel_path_surface_bounce_pdf, bsdf_pdf);
#endif
#endif
#ifdef WITH_STAT_ALL
#ifdef shader_bsdf_sample_eval_light_pass
      {
          uint32_t lightpass = UINT(bsdf_eval.use_light_pass);
          STAT_DUMP(shader_bsdf_sample_eval_light_pass, lightpass);
      }
#endif
#endif
    if (bsdf_pdf == 0.0f || bsdf_eval_is_zero(&bsdf_eval))
...
#ifdef WITH_STAT_ALL
    STAT_DUMP(kernel_path_surface_bounce_thr, *throughput);
#endif
