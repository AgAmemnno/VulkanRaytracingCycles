                                 ...
                                 const Ray *ray)
{
  ...
  if (sd->type == PRIMITIVE_TRIANGLE) {
    ...
    STAT_DUMP(shader_setup_from_subsurface_P, sd->P);
#endif
...
#  endif
  ...
  STAT_DUMP(shader_setup_from_subsurface_I, sd->I);
#endif
...
ccl_device_inline void shader_prepare_closures(ShaderData *sd, ccl_addr_space PathState *state)
{
  ...
  auto f = make_float3(sum, (float)state->bounce, (float)sd->num_closure);
  STAT_DUMP(shader_prepare_closures_sum, f);
#endif
...
#ifdef  WITH_STAT_ALL
  STAT_DUMP(shader_bsdf_multi_eval_sum_no_mis, result_eval->sum_no_mis);
#endif
}
...
#ifdef  WITH_STAT_ALL
  STAT_DUMP(shader_bsdf_sample_eval,eval);
#endif
...
#ifdef  WITH_STAT_ALL
    STAT_DUMP(shader_bsdf_sample_eval_diffuse, bsdf_eval->diffuse);
    if (bool(kernel_data.film.use_light_pass)) {
        STAT_DUMP(shader_bsdf_sample_eval_glossy, bsdf_eval->glossy);
        STAT_DUMP(shader_bsdf_sample_eval_transmission, bsdf_eval->transmission);
        STAT_DUMP(shader_bsdf_sample_eval_transparent, bsdf_eval->transparent);
    }
#endif
