...
int ImageHandle::slot_back() const
{
    if (0 == tile_slots.size()) {
        return -1;
    }
    return tile_slots.back();
}
...
#ifdef __VULKAN_DEV__


void ImageManager::device_load_image2(int slot)
{
    Image* img = images[slot];

    printf("Updating Images   Loading  %s  \n" , img->loader->name().c_str());

    const int texture_limit = 0;
    ...
    img->mem = new device_texture(
         nullptr, img->mem_name.c_str(), slot, type, img->params.interpolation, img->params.extension);
...
void ImageManager::device_update2()
{
    if (!need_update) {
        return;
    }
...
ImageHandle ImageManager::add_image2(const string& filename, const ImageParams& params)
{
    const int slot = add_image_slot2(new OIIOImageLoader(filename), params, false);
...
ImageHandle ImageManager::add_image2(const string& filename,
    const ImageParams& params,
    const vector<int>& tiles)
{
    ImageHandle handle;
    ...
    foreach(int tile, tiles) {
        ...
        const int slot = add_image_slot2(new OIIOImageLoader(tile_filename), params, false);
        handle.tile_slots.push_back(slot);
    }
...
int ImageManager::add_image_slot2(ImageLoader* loader,
    const ImageParams& params,
    const bool builtin)
{
    Image* img;
...
ImageManager::Image* ImageManager::get_image(int slot) {
    if (images.size() > slot) {
        return images[slot];
    }
    return (ImageManager::Image*)0;
};
#endif
