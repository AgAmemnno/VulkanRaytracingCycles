...
void SVMCompiler::generate_node(ShaderNode *node, ShaderNodeSet &done)
{
   ...
   if (std::string(node->type->name) == "environment_texture") {
      redis->write_tex_nodes((EnvironmentTextureNode*)node, "EnvironmentTextureNode");
   }else    if (std::string(node->type->name) == "image_texture") {
       redis->write_tex_nodes((ImageTextureNode*)node, "ImageTextureNode");
   }
