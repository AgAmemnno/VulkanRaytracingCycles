...
class ImageManager {
 public:
  ...
  void device_update2();
  void device_load_image2(int slot);
  Image* get_image(int slot);
  ImageHandle add_image2(const string& filename, const ImageParams& params);
  int add_image_slot2(ImageLoader* loader,const ImageParams& params,const bool builtin = true);
  ImageHandle add_image2(const string& filename,
      const ImageParams& params,
      const vector<int>& tiles);
#endif
 private:
