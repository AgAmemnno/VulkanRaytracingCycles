...
void Geometry::compute_bvh_optix(
    Device* device, DeviceScene* dscene, SceneParams* params, Progress* progress, int n, int total)
{
    if (progress->get_cancel())
        return;
    ...
    const BVHLayout bvh_layout = params->bvh_layout;
  ...
  for (size_t i = 0; i < scene->objects.size(); i++) {
    ...
    foreach (AttributeRequest &req, attributes.requests) {
      ...
      if (req.desc.element != ATTR_ELEMENT_NONE) {
        ...
        if (req.type ==  TypeFloat)
          osl_attr.type =  TypeFloat;
        else if (req.type ==  TypeMatrix)
          osl_attr.type =  TypeMatrix;
        else if (req.type == TypeFloat2)
          osl_attr.type = TypeFloat2;
        else if (req.type == TypeRGBA)
          osl_attr.type = TypeRGBA;
        else
          osl_attr.type =  TypeColor;
      ...
      if (req.subd_desc.element != ATTR_ELEMENT_NONE) {
        ...
        if (req.subd_type ==  TypeFloat)
          osl_attr.type =  TypeFloat;
        else if (req.subd_type ==  TypeMatrix)
          osl_attr.type =  TypeMatrix;
...
void GeometryManager::device_steal_bvh_optix(Device *device,
                                        DeviceScene *dscene,
                                        Scene *scene, vector<Geometry*>& geom,
                                        Progress &progress)
{
  /* bvh build */
  progress.set_status("Updating Scene BVH", "Building");
  ...
  BVHParams bparams;
  bparams.top_level = true;
  bparams.bvh_layout = BVH_LAYOUT_OPTIX;//HParams::best_bvh_layout(scene->params.bvh_layout,
                                                  //vice->get_bvh_layout_mask());
  bparams.use_spatial_split = scene->params.use_bvh_spatial_split;
  bparams.use_unaligned_nodes = dscene->data.bvh.have_curves &&
                                scene->params.use_bvh_unaligned_nodes;
  bparams.num_motion_triangle_steps = scene->params.num_bvh_time_steps;
  ...
  BVH *bvh = BVH::create(bparams, geom, scene->objects);
  bvh->build(progress, &device->stats);
  ...
  if (pack.nodes.size()) {
    dscene->bvh_nodes.steal_data(pack.nodes);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->bvh_nodes);
#endif
  ...
  if (pack.leaf_nodes.size()) {
    dscene->bvh_leaf_nodes.steal_data(pack.leaf_nodes);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->bvh_leaf_nodes);
#endif
  }
  if (pack.object_node.size()) {
    dscene->object_node.steal_data(pack.object_node);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->object_node);
#endif
  }
  if (pack.prim_tri_index.size()) {
    dscene->prim_tri_index.steal_data(pack.prim_tri_index);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_tri_index);
#endif
  }
  if (pack.prim_tri_verts.size()) {
    dscene->prim_tri_verts.steal_data(pack.prim_tri_verts);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_tri_verts);
#endif
  }
  if (pack.prim_type.size()) {
    dscene->prim_type.steal_data(pack.prim_type);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_type);
#endif
  }
  if (pack.prim_visibility.size()) {
    dscene->prim_visibility.steal_data(pack.prim_visibility);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_visibility);
#endif
  }
  if (pack.prim_index.size()) {
    dscene->prim_index.steal_data(pack.prim_index);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_index);
#endif
  }
  if (pack.prim_object.size()) {
    dscene->prim_object.steal_data(pack.prim_object);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_object);
#endif
  }
  if (pack.prim_time.size()) {
    dscene->prim_time.steal_data(pack.prim_time);
#ifdef DUMP_GEOM
    redis->kg_textures(dscene->prim_time);
#endif
  }
                                    ...
                                    Progress &progress)
{
  ...
  static vector<Geometry*>  dump_geom;
  dump_geom.clear();
  DeviceScene dump_scene(device);
...
#ifdef OPTIX_DEBUG
          Mesh* geom_copy = new Mesh();
          memcpy(geom_copy, geom, sizeof(Mesh));
          dump_geom.push_back(geom_copy);
#endif
  ...
  auto layout = scene->params.bvh_layout;
  i = 0;
 TaskPool pool2;
 scene->params.bvh_layout = BVH_LAYOUT_OPTIX;
  foreach(Geometry* geom, dump_geom) {
      if (geom->need_update) {
          pool2.push(function_bind(
              &Geometry::compute_bvh_optix, geom , device, dscene, &scene->params, &progress, i, num_bvh));
          if (geom->need_build_bvh(BVH_LAYOUT_OPTIX)) {
              i++;
          }
      }
  }

  TaskPool::Summary summary2;
  pool2.wait_work(&summary2);
  VLOG(2) << "Objects BVH build pool statistics:\n" << summary2.full_report();

  scene->params.bvh_layout = layout;
#endif
...
#ifdef OPTIX_DEBUG
  device_steal_bvh_optix(device, &dump_scene, scene,dump_geom, progress);
#endif
...
#ifdef DUMP_GEOM
  redis->FlushDB(4);
  redis->write_geoms(scene->geometry);
  redis->write_objects(scene->objects);
#endif
...
void GeometryManager::device_update_optix(Device* device,
    DeviceScene* dscene,
    Scene* scene,
    Progress& progress)
{
