...
#define PNGSUITE_PRIMARY
    ...
    void test1() {
        ...
        for (i = 0; i < stb_arr_len(files); ++i) {
            ...
            printf(".");
            //printf("%s\n", files[i]);
            data = stbi_load(files[i], &w, &h, &n, 0); if (data) free(data); else stb_arr_push(failed, (char*)"&n");
            data = stbi_load(files[i], &w, &h, 0, 1); if (data) free(data); else stb_arr_push(failed, (char*)"1");
            data = stbi_load(files[i], &w, &h, 0, 2); if (data) free(data); else stb_arr_push(failed, (char*)"2");
            data = stbi_load(files[i], &w, &h, 0, 3); if (data) free(data); else stb_arr_push(failed, (char*)"3");
            data = stbi_load(files[i], &w, &h, 0, 4); if (data); else stb_arr_push(failed, (char*)"4");
            if (data) {
                char fname[512];
