...
void main()
{
    ...
    uint Dx = gl_LaunchSizeNV.x;
    uint Dy = gl_LaunchSizeNV.y;
    PROFI_IDX = int(gl_LaunchIDNV.x + (gl_LaunchIDNV.y * Dx));
    int atomic_offset = int(((((gl_BuiltIn_5377 * gl_BuiltIn_5374) + gl_BuiltIn_5376) * gl_SubgroupSize) + gl_SubgroupInvocationID) * 64u);
    push.pool_ptr.pool_sc.data[PROFI_IDX + 2228224].weight = vec4(float(atomic_offset), float(gl_BuiltIn_5377), float(gl_BuiltIn_5376), float(gl_SubgroupInvocationID));
    bool _303 = gl_LaunchIDNV.x == 0u;
    bool _309;
    if (_303)
    {
        _309 = gl_LaunchIDNV.y == 0u;
    }
    else
    {
        _309 = _303;
    ...
    {
        PROFI_IDX = int(Dx * Dy);
        push.pool_ptr.pool_sc.data[PROFI_IDX + 2228224].weight = vec4(float(gl_BuiltIn_5375), float(gl_BuiltIn_5374), float(gl_SubgroupSize), 64.0);
    }
}
