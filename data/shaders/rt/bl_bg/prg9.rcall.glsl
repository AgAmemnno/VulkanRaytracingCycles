...
{
    return vec4(dot(float4_to_float3(_1300.kernel_data.film.xyz_to_r), xyz), dot(float4_to_float3(_1300.kernel_data.film.xyz_to_g), xyz), dot(float4_to_float3(_1300.kernel_data.film.xyz_to_b), xyz), 0.0);
}
...
{
    return 2.0 * atan(sqrt(dot(a - b, a - b)), sqrt(dot(a + b, a + b)));
}
...
{
    TextureInfo _482;
    _482.data = push.data_ptr._texture_info.data[id].data;
    _482.data_type = push.data_ptr._texture_info.data[id].data_type;
    _482.cl_buffer = push.data_ptr._texture_info.data[id].cl_buffer;
    _482.interpolation = push.data_ptr._texture_info.data[id].interpolation;
    _482.extension = push.data_ptr._texture_info.data[id].extension;
    _482.width = push.data_ptr._texture_info.data[id].width;
    _482.height = push.data_ptr._texture_info.data[id].height;
    _482.depth = push.data_ptr._texture_info.data[id].depth;
    _482.use_transform_3d = push.data_ptr._texture_info.data[id].use_transform_3d;
    _482.transform_3d.x = push.data_ptr._texture_info.data[id].transform_3d.x;
    _482.transform_3d.y = push.data_ptr._texture_info.data[id].transform_3d.y;
    _482.transform_3d.z = push.data_ptr._texture_info.data[id].transform_3d.z;
    _482.pad[0] = push.data_ptr._texture_info.data[id].pad[0];
    _482.pad[1] = push.data_ptr._texture_info.data[id].pad[1];
    TextureInfo info = _482;
    ...
    if ((((texture_type == IMAGE_DATA_TYPE_FLOAT4) || (texture_type == IMAGE_DATA_TYPE_BYTE4)) || (texture_type == IMAGE_DATA_TYPE_HALF4)) || (texture_type == IMAGE_DATA_TYPE_USHORT4))
    {
        if (info.interpolation == INTERPOLATION_CUBIC)
        {
            ...
            uint _860 = texSlot;
            uint _864 = sampID;
            vec4 ret = (((textureLod(sampler2D(_tex_[nonuniformEXT(_789)], _samp_[nonuniformEXT(_798)]), vec2(x0, y0), 0.0) * g0x) + (textureLod(sampler2D(_tex_[nonuniformEXT(_811)], _samp_[nonuniformEXT(_815)]), vec2(x1, y0), 0.0) * g1x)) * ((0.16666667163372039794921875 * ((fy * ((fy * ((-fy) + 3.0)) - 3.0)) + 1.0)) + (0.16666667163372039794921875 * (((fy * fy) * ((3.0 * fy) - 6.0)) + 4.0)))) + (((textureLod(sampler2D(_tex_[nonuniformEXT(_845)], _samp_[nonuniformEXT(_849)]), vec2(x0, y1), 0.0) * g0x) + (textureLod(sampler2D(_tex_[nonuniformEXT(_860)], _samp_[nonuniformEXT(_864)]), vec2(x1, y1), 0.0) * g1x)) * ((0.16666667163372039794921875 * ((fy * ((fy * (((-3.0) * fy) + 3.0)) + 3.0)) + 1.0)) + (0.16666667163372039794921875 * ((fy * fy) * fy))));
            return ret;
        }
        else
        {
            ...
            uint _893 = texSlot;
            uint _897 = sampID_1;
            return textureLod(sampler2D(_tex_[nonuniformEXT(_893)], _samp_[nonuniformEXT(_897)]), vec2(x, y), 0.0);
        }
    }
    else
    {
        ...
        if (info.interpolation == INTERPOLATION_CUBIC)
        {
            ...
            uint _1221 = texSlot;
            uint _1225 = sampID_2;
            f = (((0.16666667163372039794921875 * ((fy_1 * ((fy_1 * ((-fy_1) + 3.0)) - 3.0)) + 1.0)) + (0.16666667163372039794921875 * (((fy_1 * fy_1) * ((3.0 * fy_1) - 6.0)) + 4.0))) * ((g0x_1 * textureLod(sampler2D(_tex_[nonuniformEXT(_1154)], _samp_[nonuniformEXT(_1158)]), vec2(x0_1, y0_1), 0.0).x) + (g1x_1 * textureLod(sampler2D(_tex_[nonuniformEXT(_1170)], _samp_[nonuniformEXT(_1174)]), vec2(x1_1, y0_1), 0.0).x))) + (((0.16666667163372039794921875 * ((fy_1 * ((fy_1 * (((-3.0) * fy_1) + 3.0)) + 3.0)) + 1.0)) + (0.16666667163372039794921875 * ((fy_1 * fy_1) * fy_1))) * ((g0x_1 * textureLod(sampler2D(_tex_[nonuniformEXT(_1205)], _samp_[nonuniformEXT(_1209)]), vec2(x0_1, y1_1), 0.0).x) + (g1x_1 * textureLod(sampler2D(_tex_[nonuniformEXT(_1221)], _samp_[nonuniformEXT(_1225)]), vec2(x1_1, y1_1), 0.0).x)));
        }
        else
        {
            ...
            uint _1253 = texSlot;
            uint _1257 = sampID_3;
            f = textureLod(sampler2D(_tex_[nonuniformEXT(_1253)], _samp_[nonuniformEXT(_1257)]), vec2(x, y), 0.0).x;
        }
...
vec4 sky_radiance_nishita(vec4 dir, float nishita_data[10], uint texture_id)
{
    ...
    if (dir.z >= 0.0)
    {
        ...
        {
            vec4 pixel_bottom = vec4(nishita_data[0], nishita_data[1], nishita_data[2], 0.0);
            vec4 pixel_top = vec4(nishita_data[3], nishita_data[4], nishita_data[5], 0.0);
            float y;
            if ((sun_elevation - half_angular) > 0.0)
            {
                if ((sun_elevation + half_angular) > 0.0)
                {
                    ...
                    xyz = mix(pixel_bottom, pixel_top, vec4(y)) * sun_intensity;
                }
            }
            else
            {
        ...
        else
        {
            ...
            float param_8 = x;
            float param_9 = y_1;
            vec4 _1764 = kernel_tex_image_interp(param_7, param_8, param_9);
            xyz = float4_to_float3(_1764);
        }
    }
    else
    {
        ...
        else
        {
            ...
            float param_12 = x_1;
            float param_13 = -0.5;
            vec4 _1805 = kernel_tex_image_interp(param_11, param_12, param_13);
            xyz = float4_to_float3(_1805) * fade;
        }
    }
    vec4 param_14 = xyz;
    return xyz_to_rgb(param_14);
}
...
void svm_node_tex_sky()
{
    ...
    if ((sky_model == 0) || (sky_model == 1))
    {
        ...
        if (sky_model == 0)
        {
            ...
            float param_8[9] = config_z;
            vec4 _2165 = sky_radiance_preetham(param, param_1, param_2, param_3, param_4, param_5, param_6, param_7, param_8);
            config_x = param_6;
            config_y = param_7;
            config_z = param_8;
            f = _2165;
        ...
        else
        {
            ...
            vec4 _2189 = sky_radiance_hosek(param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16, param_17);
            config_x = param_15;
            config_y = param_16;
            config_z = param_17;
            f = _2189;
...
vec2 direction_to_equirectangular_range(vec4 dir, vec4 range)
{
    ...
    float v = (acos(dir.z / sqrt(dot(dir, dir))) - range.w) / range.z;
    return vec2(u, v);
}
...
float color_srgb_to_linear(float c)
{
    if (c < 0.040449999272823333740234375)
    {
        ...
        {
            _283 = c * 0.077399380505084991455078125;
        }
        return _283;
    }
    else
...
vec4 svm_image_texture(int id, float x, float y, uint flags)
{
    ...
    float param_1 = x;
    float param_2 = y;
    vec4 _2329 = kernel_tex_image_interp(param, param_1, param_2);
    vec4 r = _2329;
    float alpha = r.w;
...
void svm_node_tex_environment()
{
    ...
    {
        vec4 param_1 = co;
        vec2 _2459 = direction_to_mirrorball(param_1);
        uv = _2459;
    }
